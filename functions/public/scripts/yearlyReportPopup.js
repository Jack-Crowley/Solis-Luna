// Annual report popup (once per session)
(function yearlyReportPopup() {
  ...
  const SESSION_FLAG = 'yearlyReportPopupShown';

  // Skip if already shown
  try { if (sessionStorage.getItem(SESSION_FLAG)) return; } catch (_) {}
  ...
  function dismiss(setFlag) {
    ...
    if (setFlag) { try { sessionStorage.setItem(SESSION_FLAG,'1'); } catch (_) {} }
  }
})();
    if (!document.getElementById(STYLE_ID)) {
      const styleEl = document.createElement('style');
      ...
      styleEl.textContent = `
        .yearly-report-popup-overlay {
          ...
          z-index: 9999;
          pointer-events: none; /* Let inner popup mediate interactions */
        }
        .yearly-report-popup {
          width: 100%;
          max-width: 980px;
          ...
          animation: yrp-slide-in 450ms cubic-bezier(.16,.84,.44,1);
        }
        .yearly-report-popup a.report-link {
          color: #fad783;
          font-size: 1.25rem;
          font-weight: 600;
          text-decoration: none;
        }
        .yearly-report-popup a.report-link:hover,
        .yearly-report-popup a.report-link:focus {
          text-decoration: underline;
        }
        .yearly-report-popup button.yrp-close {
          position: absolute;
          top: 8px;
          right: 12px;
        ...
        .yearly-report-popup button.yrp-close:focus {
          outline: 2px solid #fad783;
          outline-offset: 2px;
        }
        ...
        @keyframes yrp-fade-out {
          to {
            opacity: 0;
            transform: translateY(-8px);
          }
        }
        @keyframes yrp-slide-in {
          from {
            opacity: 0;
    ...
    // Link (force download behavior is added in handler below)
    const link = document.createElement('a');
    link.href = REPORT_URL;
    link.className = 'report-link';
    link.textContent = 'Click here to view our yearly report';
    ...
    link.addEventListener('click', (e) => {
      ...
      // Temporary anchor to trigger download (adds cache bust param)
      const tempAnchor = document.createElement('a');
      tempAnchor.href = REPORT_URL + (REPORT_URL.includes('?') ? '&' : '?') + 'cacheBust=' + Date.now();
      tempAnchor.download = 'SLA-Annual-Report.pdf';
    ...
    function dismiss(setFlag) {
      ...
      setTimeout(() => overlay.remove(), 240);
      if (setFlag) {
        try { sessionStorage.setItem(SESSION_FLAG, '1'); } catch (err) { /* ignore */ }
      }
    }