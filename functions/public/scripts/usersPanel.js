...
document.querySelector(".searchIcons").addEventListener('click', () => {
    ...
    fetch('/admin/users/search', options)
        ...
        .then(data => {
            ...
            data.forEach((user) => {
                ...
                switch (user.position) {
                    case "Regional Webmaster":
                        position.classList.add("regionWM")
                        break;
                    case "Regional Blog Manager":
                        position.classList.add("regionBM")
                        break;
                    case "Regional Director":
                        position.classList.add("regionD")
                        break;
                    default:
                        position.classList.add(user.position.toLowerCase())
                        break;
                }

                let date = document.createElement('td')
                date.textContent = getFormattedDate(new Date((user.created["_seconds"] * 1000)+ (user.created["_nanoseconds"] / 1000000)))
                ...
                edit.addEventListener('click', (e) => {
                    window.location.href = "/admin/users/edit/"+e.target.dataset.uid
                })
            
                dele.addEventListener('click', (e) => {
                    window.location.href = "/admin/users/delete/"+e.target.dataset.uid
                })
